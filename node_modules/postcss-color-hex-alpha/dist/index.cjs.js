...
var Func = _interopDefault(require('postcss-values-parser/lib/nodes/Func'));
var Punctuation = _interopDefault(require('postcss-values-parser/lib/nodes/Punctuation'));
...
const onCSSDeclaration = decl => {
  if (hasAlphaHex(decl)) {
    const {
      value: originalValue
    } = decl; // replace instances of hexa with rgba()
...
const hexa2rgba = node => {
  ...
  const [r, g, b, a] = [parseInt(hex8.slice(2, 4), 16), parseInt(hex8.slice(4, 6), 16), parseInt(hex8.slice(6, 8), 16), Math.round(parseInt(hex8.slice(8, 10), 16) / 255 * alphaDecimalPrecision) / alphaDecimalPrecision]; // return a new rgba function, preserving the whitespace of the original node

  const rgbaFunc = Object.assign(new Func({
    name: 'rgba',
    raws: {}
  }), {
    raws: node.raws
  ...
  rgbaFunc.append(new Punctuation({
    value: ','
  }));
...
const createNumberNode = number => postcssValuesParser.parse(number).first;
/** @typedef {import('postcss').Declaration} CSSDeclaration */

/** Transform 4 & 8 character hex color notation in CSS. */

const postcssPlugin = postcss.plugin('postcss-color-hex-alpha',
